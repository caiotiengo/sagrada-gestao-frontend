...
import { Calendar, Home, Info, Loader2, Ticket, Trophy } from 'lucide-react'
...
export default function PublicRafflePage({ params }: PageProps) {
  ...
  // Build a set of taken (unavailable) numbers from takenNumbers
  ...
  if (isError || !raffle) {
    return (
      <PublicLayout>
        <ErrorState
          title="Rifa n&atilde;o encontrada"
          message="N&atilde;o foi poss&iacute;vel carregar os dados da rifa."
          onRetry={refetch}
        />
      </PublicLayout>
    )
      ...
      <div className="bg-gradient-to-br from-primary to-primary/80">
        <div className="mx-auto flex max-w-4xl items-center gap-3 px-4 py-4 sm:px-6 sm:py-5">
          <Avatar size="lg" className="border-2 border-white/30">
            {raffle.house.photoUrl && (
              <AvatarImage src={raffle.house.photoUrl} alt={raffle.house.name} />
            )}
            ...
            <div className="min-w-0 text-sm">
              ...
              {raffle.winnerNumber != null && (
                <p className="text-amber-800 dark:text-amber-200">
                  Número ganhador: {String(raffle.winnerNumber).padStart(2, '0')}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mb-6 flex items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3">
            ...
            <div className="min-w-0 text-sm">
              ...
              <p className="text-muted-foreground">
                Não é mais possível reservar números.
              </p>
            </div>
          </div>
        )}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">
                  {isClosed ? 'Números' : 'Escolha seus números'}
                </CardTitle>
                {!isClosed && (
                  <CardDescription>
                  ...
                  {raffle.status === 'drawn' ? (
                    <>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          Esta rifa já foi sorteada e não aceita novas reservas.
                        </p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          Esta rifa foi cancelada e não aceita novas reservas.
                        </p>
                      </div>
                    </>
                  )}
                    ...
                    {totalSelected > 0 && (
                      <div className="rounded-lg bg-primary/5 p-3 text-sm">
                        <p className="font-medium">
                          {totalSelected} número{totalSelected > 1 ? 's' : ''}
                        </p>
                        <p className="text-muted-foreground">
