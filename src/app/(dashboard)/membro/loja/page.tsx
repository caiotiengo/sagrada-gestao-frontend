...
export default function MemberLojaPage() {
  return (
...
function StoreTab() {
  ...
  const { data, isLoading, isError, refetch } = useStoreItems(1, 'store')
  ...
  if (isLoading) return <LoadingState message="Carregando loja..." />
    ...
    <div className="space-y-4">
      ...
      {items.length > 0 ? (
        ...
        <EmptyState
          ...
          description="Não há produtos disponíveis na loja no momento."
        />
      )}
...
function OrdersTab() {
  ...
  const { data, isLoading, isError, refetch } = useMemberSales(memberId, page, 'store')
  ...
  if (sales.length === 0) {
    return (
      <EmptyState
        icon={Package}
        title="Nenhum pedido"
        description="Você ainda não fez nenhum pedido na loja."
      />
    )
  }
